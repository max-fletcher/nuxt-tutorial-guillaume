...
<script setup lang="ts">
  ...
  // However, you can just use 'useFetch' for submitting POST & PUT/PATCH requests by providing a 2nd argument inside 'useFetch'.
  ...
  // 'useLazyFetch' is the same as 'useFetch' and 'useLazyAsyncData' is the same as 'useAsyncData' but with the option- lazy:true. It means that the API call is not ran on server. Any 'useFetch'/'useAsyncData'/'$fetch' are ran
  // on server and the page is hydrated based on the data received and sent to the client. After that, if there are any 'useLazyFetch'/'useLazyAsyncData' are present, it will be called on the client end. This also means you
  // have to handle what happens before the API call finishes(i.e when data is null because the API call is not done yet.). These are used in say infinite scrolling or when you want to show the user the page first(e.g skeleton) 
  // and meanwhile load heavy data that takes time to load(i.e better UX).
